...
# Seed: 1000
ENV_ID='BeamRider-v0'; NUM_OPTIONS=5; SEED=1000; GPU_DEVICES='0'; tmux new -d -s "train/option/env_id-$ENV_ID/seed-$SEED" "mkdir -p results/env_id-$ENV_ID/seed-$SEED; python -m option.main --env_id=$ENV_ID --num_options=$NUM_OPTIONS --seed=$SEED --gpu_devices=$GPU_DEVICES > results/env_id-$ENV_ID/seed-$SEED/stdout.log 2> results/env_id-$ENV_ID/seed-$SEED/stderr.log"
